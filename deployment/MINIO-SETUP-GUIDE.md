...
## Custom Folder Permissions (Per-Prefix Access Control)

MinIO supports granular permissions per folder/prefix. Ini berguna untuk membatasi akses berdasarkan agency, supplier, atau document type.

### Example: Agency-Specific Folder Access

```bash
# Create policy untuk agency tertentu (hanya bisa akses folder mereka sendiri)
cat > /tmp/agency-policy.json <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
      "Effect": "Allow",
      "Principal": {"AWS": ["arn:aws:iam::minioadmin:user/agency-001"]},
      "Action": [
        "s3:GetObject",
        "s3:PutObject",
        "s3:DeleteObject"
      ],
      "Resource": ["arn:aws:s3:::tour-travel-documents/agencies/agency-001/*"]
    },
    {
      "Effect": "Allow",
      "Principal": {"AWS": ["arn:aws:iam::minioadmin:user/agency-001"]},
      "Action": ["s3:ListBucket"],
      "Resource": ["arn:aws:s3:::tour-travel-documents"],
      "Condition": {
        "StringLike": {
          "s3:prefix": ["agencies/agency-001/*"]
        }
      }
    }
  ]
...
# Apply policy
mc anonymous set-json /tmp/agency-policy.json local/tour-travel-documents
```

### Example: Document Type Folder Access (Read-Only)

```bash
# Policy untuk public documents (read-only)
cat > /tmp/public-docs-policy.json <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
      ...
      "Principal": {"AWS": ["*"]},
      "Action": ["s3:GetObject"],
      "Resource": ["arn:aws:s3:::tour-travel-documents/public-documents/*"]
    },
    {
      "Effect": "Allow",
      "Principal": {"AWS": ["*"]},
...
mc anonymous set-json /tmp/public-docs-policy.json local/tour-travel-documents
```

### Example: Supplier KYC Documents (Write-Only for Upload)

```bash
# Policy untuk supplier upload KYC (write-only)
cat > /tmp/supplier-kyc-policy.json <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
      "Effect": "Allow",
      "Principal": {"AWS": ["arn:aws:iam::minioadmin:user/supplier-upload"]},
      "Action": [
        "s3:PutObject",
        "s3:GetObject"
      ],
      "Resource": ["arn:aws:s3:::tour-travel-documents/kyc/suppliers/*"]
    }
  ]
}
EOF

mc anonymous set-json /tmp/supplier-kyc-policy.json local/tour-travel-documents
```
...
### Create Access Keys with Folder Restrictions

```bash
# 1. Create access key via MinIO Console
# Go to: http://YOUR_SERVER_IP:9001 → Access Keys → Create access key

# 2. Or create via mc with policy
cat > /tmp/agency-001-policy.json <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:s3:::tour-travel-documents/agencies/agency-001/*",
        "arn:aws:s3:::tour-travel-documents"
      ]
    }
  ]
}
...
# 3. Create user with policy
mc admin user add local agency-001-user agency-001-password
mc admin policy create local agency-001-policy /tmp/agency-001-policy.json
mc admin policy attach local agency-001-policy --user=agency-001-user
```

### Backend Implementation (C#)

```csharp
// In your MinIO service, use presigned URLs with folder restrictions
public async Task<string> GeneratePresignedUrl(string agencyId, string documentPath, int expirationMinutes = 60)
{
    ...
    // Ensure path is within agency folder
    var safePath = $"agencies/{agencyId}/{documentPath}";
...
// Upload dengan folder restriction
public async Task<string> UploadDocument(string agencyId, string documentType, IFormFile file)
{
    var minio = new MinioClient()
        .WithEndpoint(_endpoint)
        .WithCredentials(_accessKey, _secretKey)
    ...
    var objectName = $"agencies/{agencyId}/{documentType}/{Guid.NewGuid()}_{file.FileName}";
...
# Get policy details
mc admin policy info local agency-001-policy

# Test access with specific credentials
mc alias set agency-001 http://localhost:9000 agency-001-user agency-001-password
mc ls agency-001/tour-travel-documents/agencies/agency-001/

# Should fail (access denied)
mc ls agency-001/tour-travel-documents/agencies/agency-002/
```
