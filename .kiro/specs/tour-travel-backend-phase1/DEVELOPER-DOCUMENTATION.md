...
**Reference:** Requirement 46, Design Document - API Response Format Standards

### Overview

All API endpoints follow a standardized response format with **snake_case** naming convention for consistency and REST API best practices.

### JSON Naming Convention

**All JSON properties use snake_case:**
- C# Property: `UserId` → JSON: `user_id`
- C# Property: `FullName` → JSON: `full_name`
- C# Property: `CreatedAt` → JSON: `created_at`
...
**All responses are automatically wrapped using `ApiResponseWrapperFilter`:**
...
public async Task<IActionResult> GetAgencyById(Guid id)
{
    ...
    return Ok(result);  // Automatically wrapped!
}

// Actual response (automatically wrapped)
{
  "success": true,
  "data": {
    "id": "...",
...
**Benefits:**
- ✅ Zero manual wrapping in controllers
...
**2. Filters:**
- `ApiResponseWrapperFilter` - Automatically wraps all success responses
- Intercepts `Ok()`, `Created()`, `NoContent()` results
- Skips non-API endpoints (Swagger, Health checks)

**3. Middleware:**
- `GlobalExceptionHandlerMiddleware` - Handles all exceptions
...
- Response wrapped in `{ success, data, message, timestamp }`
- Error responses have `{ success: false, error: { code, message, details } }`
